...
export interface IChatRepository {
  ...
  findManyByCompanyIdAndProvider(
    companyId: string,
    provider: string,
  ): Promise<ChatChannel[]>;
  findManyByCompanyIdAndProviderWithAccess(
    companyId: string,
    provider: string,
    userId: string,
  ): Promise<ChatChannel[]>;
...
export class ChatRepository implements IChatRepository {
  ...
  async listConversations(
    ...
    const take = searchParams?.limit
      ? parseInt(String(searchParams.limit))
      : undefined;
  ...
  async upsertConversation(data: any): Promise<ChatConversation> {
    ...
    return this.prisma.chatConversation.create({
      data: {
        ...
        unread_count:
          typeof rest.unread_count === 'object' ? 1 : rest.unread_count || 0,
      },
    }) as any;
  }
  ...
  async findManyByCompanyIdAndProvider(
    companyId: string,
    provider: string,
    userRole?: string,
  ): Promise<ChatChannel[]> {
    const where: any = { provider: provider, deleted_at: null };
  ...
  async findManyByCompanyIdAndProviderWithAccess(
    companyId: string,
    provider: string,
    userId: string,
  ): Promise<ChatChannel[]> {
    ...
    return this.prisma.chatChannel.findMany({
      where: {
        ...
        AND: [
          {
            OR: [
              { chat_channel_members: { none: { deleted_at: null } } },
              {
  ...
  async listQuickResponses(companyId: string): Promise<QuickResponse[]> {
    return this.prisma.chatQuickResponse.findMany({
  ...
  async createQuickResponse(
    ...
    return this.prisma.chatQuickResponse.create({
      data: { ...data, company_id: companyId },
    }) as any;
  }

  async updateQuickResponse(
    id: string,
    companyId: string,
    data: any,
  ): Promise<QuickResponse> {
    return this.prisma.chatQuickResponse.update({
