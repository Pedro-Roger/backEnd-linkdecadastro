...
export class ChatService {
  ...
  async connectChannel(
    ...
    companyId: string,
    channelId?: string,
  ): Promise<ConnectInstanceResponseDto> {
    ...
    if (channelId) {
      ...
      if (channel) {
        ...
        const availableInstance =
          await this.evolutionInstancesService.getAvailableInstance(
            'EVOLUTION',
          );
        const id = uuidv4();
    ...
    const createdData = await this.provider.createInstance(
      instanceName,
      channel.instance_server_id,
    );
  ...
  async getStatus(
    ...
    if (channel.status === 'connecting' || channel.status === 'connected') {
      const details = await this.provider.getInstanceDetails(
        channel.instance_name,
        channel.instance_server_id,
      );
      const s =
        details?.connectionStatus === 'open' ? 'connected' : 'disconnected';
      if (s !== channel.status) {
  ...
  async sendMessage(
    ...
    return await this.provider.sendTextMessage(
      channel.instance_name,
      phoneNumber,
      message,
      channel.instance_server_id,
  ...
  async disconnectByChannelId(
    channelId: string,
  ): Promise<{ success: boolean }> {
    const channel = await this.repository.findById(channelId);
    if (!channel) throw new NotFoundException('Channel not found');
    await this.provider.deleteInstance(
      channel.instance_name,
      channel.instance_server_id,
    );
  ...
  async disconnect(
    ...
    return channelId
      ? this.disconnectByChannelId(channelId)
      : { success: false };
  }
  ...
  async createChannel(
    ...
    const availableInstance =
      await this.evolutionInstancesService.getAvailableInstance('EVOLUTION');
  ...
  async listChannels(
    ...
    return (this.repository as any).findManyByCompanyIdAndProvider(
      companyId,
      this.getProviderByType(type),
      userRole,
    );
  }
  ...
  async getConversations(
    ...
    companyId: string,
    userRole?: string,
    searchParams?: ChatConversationsQueryDto,
  ): Promise<PaginationDataDto<any>> {
    let channelIds = searchParams?.channelId ? [searchParams.channelId] : [];

    if (channelIds.length === 0) {
      const channels = await this.listChannels(type, companyId, userRole);
      channelIds = channels.map((c) => c.id);
    }

    const data = await this.repository.listConversations(
      channelIds,
      searchParams,
    );
    return {
      ...
      totalPage: Math.ceil(data[1] / (searchParams?.limit || 15)),
    };
  }
  ...
  async getMessages(
  ...
  ): Promise<PaginationDataDto<ChatMessage>> {
    ...
    return {
      ...
      totalPage: Math.ceil(data[1] / (pagination?.limit || 50)),
    };
  }
  ...
  async sendMediaMessage(
    ...
    if (!channel) throw new NotFoundException('Channel not found');
    return await this.provider.sendMediaMessage(
      channel.instance_name,
      phoneNumber,
      mediaUrl,
      mediaType,
  ...
  async listQuickResponses(companyId: string) {
    return this.repository.listQuickResponses(companyId);
  }

  async createQuickResponse(companyId: string, data: any) {
    return this.repository.createQuickResponse(companyId, data);
  }

  async updateQuickResponse(id: string, companyId: string, data: any) {
    return this.repository.updateQuickResponse(id, companyId, data);
  }

  async deleteQuickResponse(id: string, companyId: string) {
    return this.repository.deleteQuickResponse(id, companyId);
  }
}
