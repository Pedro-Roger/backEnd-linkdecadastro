...
export class InstancesServerService implements OnModuleInit {
    ...
    constructor(
        private readonly repository: InstancesServerRepository,
    ) { }
    ...
    private async refreshCache() {
        try {
            ...
            this.logger.log(`[Cache] Instâncias recarregadas. Total: ${this.cache.length}`);
        } catch (error: any) {
            this.logger.error(`Erro ao carregar instâncias para cache: ${error.message}`);
        }
    }

    async getAvailableInstance(type: string = 'EVOLUTION'): Promise<InstanceServer> {
