...
let RegistrationsService = class RegistrationsService {
    ...
    async createRegistration(data) {
        ...
        let activeClass = await this.prisma.municipalityClass.findFirst({
            where: {
                ...
                status: client_1.MunicipalityClassStatus.ACTIVE,
            },
            orderBy: {
                classNumber: 'desc',
            },
    ...
    async handleRegistration(body) {
        ...
        if (!event || event.status !== 'ACTIVE') {
            throw new common_1.NotFoundException('Evento não encontrado ou inativo');
        }
        const participantType = data.participantType === 'OUTROS'
            ? client_1.ParticipantType.ESTUDANTE
        ...
        try {
            await this.emailService.sendRegistrationEmail(data.email, data.name, event.title);
